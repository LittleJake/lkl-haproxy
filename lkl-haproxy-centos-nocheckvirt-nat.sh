...
config-redirect-1(){
echo "iptables -t nat -A PREROUTING -i $(awk '$2 == 00000000 { print $1 }' /proc/net/route) -p tcp --dport ${port1} -j DNAT --to-destination 192.168.254.2" >> redirect.sh
echo "ip6tables -t nat -A PREROUTING -i $(awk '$2 == 00000000 { print $1 }' /proc/net/route) -p tcp --dport ${port1} -j DNAT --to-destination fd00:ffff::2" >> redirect.sh
}

config-redirect-2(){
echo "iptables -t nat -A PREROUTING -i $(awk '$2 == 00000000 { print $1 }' /proc/net/route) -p tcp --dport ${port1}:${port2} -j DNAT --to-destination 192.168.254.2" >> redirect.sh
echo "ip6tables -t nat -A PREROUTING -i $(awk '$2 == 00000000 { print $1 }' /proc/net/route) -p tcp --dport ${port1}:${port2} -j DNAT --to-destination fd00:ffff::2" >> redirect.sh
}
...
Environment=LKL_HIJACK_NET_IP=192.168.254.2
Environment=LKL_HIJACK_NET_NETMASK_LEN=24
Environment=LKL_HIJACK_NET_GATEWAY=192.168.254.1
Environment=LKL_HIJACK_NET_IPV6=fd00:ffff::2
Environment=LKL_HIJACK_NET_GATEWAY6=fd00:ffff::1
Environment=LKL_HIJACK_NET_NETMASK6_LEN=64
Environment=LKL_HIJACK_BOOT_CMDLINE=mem=32M
